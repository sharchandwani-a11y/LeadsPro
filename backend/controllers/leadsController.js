...
exports.getdashboardStats = async (req, res) => {
  try {
    ...
    // 🔥 CORE LOGIC: Agar login karne wala admin hai, toh uski apni ID hi main h.
    // Agar login karne wala normal user hai, toh uske profile me saved 'admin_id' hi uski company/network id hai.
    const currentAdminId = isAdmin ? req.user.id : req.user.admin_id;

    if (!currentAdminId) {
      return res.json({ success: true, stats: { total: 0, followups: 0, meetings: 0, converted: 0 } });
    }

    // Ab saari queries 'admin_id' ke basis par chalengi
...
// 📋 GET ALL LEADS: Naye admin ko zero data, aur user ko uske admin ka saara data dikhega
exports.getAllLeads = async (req, res) => {
  try {
    ...
    // Whichever admin owns this network
    const currentAdminId = isAdmin ? req.user.id : req.user.admin_id;
    ...
    const countQuery = query.replace('SELECT l.*, u.name AS assigned_name', 'SELECT COUNT(*) AS total');
...
exports.getLead = async (req, res) => {
  try {
    ...
    const [rows] = await db.query('SELECT * FROM leads WHERE id = ? AND admin_id = ?', [id, currentAdminId]);
...
// ➕ ADD LEAD: Lead ko hamesha current admin ke network se map karega
exports.addLead = async (req, res) => {
  try {
    const { name, email, phone, status, source, assigned_to, priority, visible_to } = req.body;
    const isAdmin = req.user.role === 'admin';
    const currentAdminId = isAdmin ? req.user.id : req.user.admin_id;

    const query = `INSERT INTO leads (name, email, phone, status, source, assigned_to, priority, visible_to, admin_id, user_id, created_at) 
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`;
    
    const [result] = await db.query(query, [
      name, email, phone, status || 'New', source || 'Website', 
      assigned_to || null, priority || 'normal', visible_to || 'all',
      currentAdminId, // Map to the business network
      req.user.id     // actual creator
    ]);

    res.status(201).json({ success: true, data: { id: result.insertId, ...req.body, admin_id: currentAdminId } });
  } catch (err) {
...
exports.updateLead = async (req, res) => {
  try {
    ...
    const query = `UPDATE leads SET 
                   ...
                   source = ?, assigned_to = ?, priority = ?, visible_to = ? 
...
exports.deleteLead = async (req, res) => {
  try {
    ...
    const [result] = await db.query('DELETE FROM leads WHERE id = ? AND admin_id = ?', [id, currentAdminId]);
